...
**命令:**
+数字 → 入款, 调用addIncomeRecord然后showAllBills
-数字 → 出款, 调用addOutgoingRecord然后showAllBills  
总账 → 调用showAllBills
删除所有账单 → 调用deleteAllRecords

**规则:**
1. 默认币种THB，有$符号用USD
2. 入款/出款后立即调用showAllBills显示账单
3. 快速提取数字直接调用工具

**示例:**
+1000 → addIncomeRecord(1000,THB) + showAllBills()
-500 → addOutgoingRecord(500,THB) + showAllBills()
总账 → showAllBills()
`,
