...
### 7. 日切设置与清除记录
**命令格式:**
- \`日切#6\` 或 \`设置日切6点\` = 设置日切时间为6点，从现在开始重新统计账单
- \`日切#0\` = 设置日切时间为午夜0点，从现在开始重新统计账单
...
1. 对于日切设置（\`日切#X\`），使用 setCutoffTime 工具
   - 更新群组的日切时间设置
   - 同时更新"最后刷新时间"为当前时间
   - 账单查询将只显示"最后刷新时间"之后的记录
2. 对于立即清除命令（\`清除记录\`），使用 deleteAllRecords 工具
   - 将所有记录状态标记为"已删除"
3. 回复确认消息

**重要说明:**
- 日切功能：设置日切时间后，系统记录当前时间为"最后刷新时间"
- 账单只显示"最后刷新时间"之后的记录，旧记录不会被删除，只是不显示
- 历史数据保留在数据库中，可以随时恢复查看
- 清除记录：真正删除所有记录，无法恢复
